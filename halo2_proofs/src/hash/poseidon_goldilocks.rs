use super::{PoseidonPermutation, SPONGE_WIDTH};
...
impl<T: Field64> PoseidonPermutation<T> {
    ...
    #[inline(always)]
    #[unroll_for_loops]
    fn mds_layer(state_: &[T; SPONGE_WIDTH]) -> [T; SPONGE_WIDTH] {
        let mut result = [T::ZERO; SPONGE_WIDTH];
        ...
        for r in 0..12 {
            if r < SPONGE_WIDTH {
                ...
                result[r] = T::from_noncanonical_u96((sum_lo, sum_hi));
            }
        }
    ...
    #[inline]
    fn full_rounds(state: &mut [T; SPONGE_WIDTH], round_ctr: &mut usize) {
    ...
    #[inline(always)]
    #[unroll_for_loops]
    fn partial_first_constant_layer(state: &mut [T; SPONGE_WIDTH]) {
        for i in 0..12 {
            if i < SPONGE_WIDTH {
                state[i] += T::from_canonical_u64(Self::FAST_PARTIAL_FIRST_ROUND_CONSTANT[i]);
            }
        }
    }
    ...
    #[inline(always)]
    #[unroll_for_loops]
    fn mds_partial_layer_init(state: &[T; SPONGE_WIDTH]) -> [T; SPONGE_WIDTH] {
        let mut result = [T::ZERO; SPONGE_WIDTH];
    ...
    #[inline(always)]
    #[unroll_for_loops]
    fn mds_partial_layer_fast(state: &[T; SPONGE_WIDTH], r: usize) -> [T; SPONGE_WIDTH] {
        ...
        for i in 1..12 {
            if i < SPONGE_WIDTH {
                let t = T::from_canonical_u64(Self::FAST_PARTIAL_ROUND_VS[r][i - 1]);
    ...
    #[inline]
    fn partial_rounds(state: &mut [T; SPONGE_WIDTH], round_ctr: &mut usize) {
    ...
    #[inline(always)]
    #[unroll_for_loops]
    fn sbox_layer(state: &mut [T; SPONGE_WIDTH]) {
        for i in 0..12 {
            if i < SPONGE_WIDTH {
    ...
    #[inline(always)]
    #[unroll_for_loops]
    fn constant_layer(state: &mut [T; SPONGE_WIDTH], round_ctr: usize) {
        for i in 0..12 {
            if i < SPONGE_WIDTH {
