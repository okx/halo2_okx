...
pub trait Transcript<C: GenericConfig, E: EncodedChallenge<C>> {
...
pub trait TranscriptRead<C: GenericConfig, E: EncodedChallenge<C>>: Transcript<C, E> {
...
pub trait TranscriptWrite<C: GenericConfig, E: EncodedChallenge<C>>: Transcript<C, E> {
...
#[derive(Debug, Clone)]
pub struct PoseidonRead<R: Read, C: GenericConfig, E: EncodedChallenge<C>> {
    // state: PoseidonState,
    reader: R,
    _marker: PhantomData<(C, E)>,
}

impl<R: Read, C: GenericConfig, E: EncodedChallenge<C>> PoseidonRead<R, C, E> {
...
/// We will replace BLAKE2b with an algebraic hash function in a later version.
#[derive(Debug, Clone)]
pub struct PoseidonWrite<W: Write, C: GenericConfig, E: EncodedChallenge<C>> {
    // state: Blake2bState,
    writer: W,
    _marker: PhantomData<(C, E)>,
}

impl<W: Write, C: GenericConfig, E: EncodedChallenge<C>> PoseidonWrite<W, C, E> {
...
pub struct ChallengeScalar<C: GenericConfig, T> {
    inner: C::Scalar,
    _marker: PhantomData<T>,
}

impl<C: GenericConfig, T> std::ops::Deref for ChallengeScalar<C, T> {
...
pub trait EncodedChallenge<C: GenericConfig> {
...
pub(crate) fn read_n_commitments<
    C: GenericConfig,
...
>(
    transcript: &mut T,
    n: usize,
) -> io::Result<Vec<C::Commitment>> {
    (0..n).map(|_| transcript.read_commitment()).collect()
}

pub(crate) fn read_n_scalars<C: GenericConfig, E: EncodedChallenge<C>, T: TranscriptRead<C, E>>(
    transcript: &mut T,
    n: usize,
