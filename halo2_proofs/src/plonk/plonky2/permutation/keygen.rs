...
use crate::plonk::plonky2::{Params, GenericConfig2};
...
impl Assembly {
    ...
    pub(crate) fn build_vk<C: GenericConfig2>(
        self,
        params: &Params,
        domain: &EvaluationDomain<C::F>,
        fixed: Vec<Polynomial<C::F, LagrangeCoeff>>,
        p: &Argument,
    ) -> VerifyingKey<C> {
        ...
        let len = fixed[0].len();
        let fixed_vec: Vec<Vec<C::F>> = (0..len).into_iter().map(|i| fixed[i].iter().map(|f| f)).collect();

        // build merkle tree for fixed polys and permutation polys
        let mut merkle_tree = MerkleTree::new(fixed_vec, params.fri_config.cap_height);
        let constants_sigmas_cap = merkle_tree.cap;

        VerifyingKey { constants_sigmas_cap }
    }

    pub(crate) fn build_pk<C: GenericConfig2>(
        self,
        params: &Params,
        domain: &EvaluationDomain<C::F>,
        p: &Argument,
