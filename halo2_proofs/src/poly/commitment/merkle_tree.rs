...
use crate::hash::{GenericHashOut, Hasher};
use crate::helpers::log2_strict;
...
pub struct MerkleCommitment<F: Field64, H: Hasher<F>>(pub Vec<H::Hash>);
...
pub struct MerkleTree<F: Field64, H: Hasher<F>> {
    ...
    /// The Merkle cap.
    pub cap: MerkleCommitment<F, H>,
}

///
impl<F: Field64, H: Hasher<F>> Commitment<F, H> for MerkleCommitment<F, H> {}

impl<F: Field64, H: Hasher<F>> Default for MerkleCommitment<F, H> {
    fn default() -> Self {
        Self(Vec::new())
    }
}

impl<F: Field64, H: Hasher<F>> MerkleCommitment<F, H> {
    ///
...
impl<F: Field64, H: Hasher<F>> Default for MerkleTree<F, H> {
    fn default() -> Self {
        Self {
            leaves: Vec::new(),
            digests: Vec::new(),
            cap: MerkleCommitment::default(),
        }
    }
}
...
fn fill_subtree<F: Field64, H: Hasher<F>>(
...
fn fill_digests_buf<F: Field64, H: Hasher<F>>(
    digests_buf: &mut [MaybeUninit<H::Hash>],
...
pub struct MerkleProof<F: Field64, H: Hasher<F>> {
    /// The Merkle digest of each sibling subtree, staying from the bottommost layer.
    pub siblings: Vec<H::Hash>,
}

///
impl<F: Field64, H: Hasher<F>> MerkleTree<F, H> {
    ///
...
mod tests {
    ...
    fn random_data<F: Field64>(n: usize, k: usize) -> Vec<Vec<F>> {
        (0..n).map(|_| F::rand_vec(k)).collect()
    }

    fn verify_all_leaves<F: Field64, C: GenericConfig, const D: usize>(
        leaves: Vec<Vec<F>>,
        cap_height: usize,
    ) -> Result<()>
    where
        <C as crate::plonk::config::GenericConfig>::Hasher: crate::hash::Hasher<F>,
    {
    ...
    fn test_cap_height_too_big() {
        ...
        type F = <C as GenericConfig>::Scalar;

        let log_n = 8;
        let cap_height = log_n + 1; // Should panic if `cap_height > len_n`.

        let leaves = random_data::<F>(1 << log_n, 7);
        let _ = MerkleTree::<F, <C as GenericConfig>::Hasher>::new(leaves, cap_height);
    }
